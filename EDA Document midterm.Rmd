...
Secondly, we design a variable called: toxicity level for human. To get the value for this variable, we first assigned the toxicity level for Carcinogen：level1: unknown, level2: probable, level3: possible, level4: known. Hormone.Disruptor：level1: unknown, level2: suspected, Neurotoxine: level1: unknown, level2: present, Developmental.or.Reproductive.Toxins: level1: unknown, level2: present. And add them to a new toxcity level range from 4 to 10, higher the toxcity level is, the more toxic the pesticide type is. We also assigned toxicity level for bee.toxins: level1: unknown, level2: slight, level3: moderate, level4: high.
...
__The Original dataframe:__

```{r, warning=FALSE}
data.frame(head(strawbwhole))
```
__After the merge and add toxicitylevel for human and bee:__

```{r, warning=FALSE}
data.frame(head(chemical))
```

__The dataset ready for EDA:__

```{r, warning=FALSE}
data.frame(head(chemical_clean))
```

__The dataset ready for maps:__

```{r, warning=FALSE}
data.frame(head(eda_subset))
```

__The subset for Total Pesticide usage__

```{r, warning=FALSE}
data.frame(head(chemicaltype_freq))
```
__The dataset for Pesticide Toxicitylevelhuman Frequency 2016:__

```{r, warning=FALSE}
data.frame(head(year1_toxicityhuman_freq))
```
...
```{r,warning=FALSE,fig.width= 10, fig.height= 6}
human_toxicity_level_function(year1_toxicityhuman_freq_full) + ggtitle("Pesticide Toxicitylevelhuman Frequency 2016")
```


```{r,warning=FALSE,fig.width= 10, fig.height= 6}
human_toxicity_level_function(year2_toxicityhuman_freq_full) + ggtitle("Pesticide Toxicitylevelhuman Frequency 2018")
```


```{r,warning=FALSE,fig.width= 10, fig.height= 6}
human_toxicity_level_function(year3_toxicityhuman_freq_full) + ggtitle("Pesticide Toxicitylevelhuman Frequency 2019")
```
From the three plots, we can see the biggest limitation for our project is that the original strawberry dataset contains too many missing value. With this major limitation, the useful sample size shrank from over 3000 observations to only around 50 observations, even among the 50 observations, there are around 60% of them are transfered from missing values. This is a serious obstacle that we encountered.

```{r,warning=FALSE,fig.width= 10, fig.height= 6}
table(chemical$measurement)
table(strawbwhole$State)
table(strawbwhole$Domain)
table(strawbwhole$Year)
```

Another limitation is about the measurement, from the table, we can see there are 5 different measurement scale in the original strawberry dataset and each has the same number of observations. This limitation shrink our sample size for studying our topic as well for we can only filter one measurement out to continue our study.
